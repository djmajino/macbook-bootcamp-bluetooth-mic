...
{
    PAGED_CODE();
#ifdef BLUETOOTH_MIC_MAC_HCI_AUDIO_BRIDGE
    CloseHciAudioStream();
    ReleaseHciAudioInterface();
#endif
    if (NULL != m_pMiniport)
    {
        if (m_pAudioModules)
...
//=============================================================================
#pragma code_seg()
BOOLEAN CMiniportWaveRTStream::IsHciAudioBridgeStream()
{
    if (m_pMiniport == NULL || m_pWfExt == NULL ||
        (m_pMiniport->m_DeviceType != eBthHfpMicDevice &&
         m_pMiniport->m_DeviceType != eBthHfpSpeakerDevice))
    {
        return FALSE;
    }

    return m_pWfExt->Format.nChannels == 1 &&
           m_pWfExt->Format.wBitsPerSample == 16 &&
           m_pWfExt->Format.nBlockAlign == 2 &&
           (m_pWfExt->Format.nSamplesPerSec == 8000 ||
            m_pWfExt->Format.nSamplesPerSec == 16000);
}

//=============================================================================
#pragma code_seg("PAGE")
VOID CMiniportWaveRTStream::AcquireHciAudioInterface()
{
    ...
    PAGED_CODE();
    if (m_HciAudioInterfaceAcquired || !IsHciAudioBridgeStream())
    {
        return;
    }

    RtlInitUnicodeString(&deviceName, L"\\Device\\UartH4Probe");
    deviceObject = NULL;
    fileObject = NULL;
    ...
    RtlZeroMemory(&m_HciAudio, sizeof(m_HciAudio));
    KeInitializeEvent(&event, NotificationEvent, FALSE);
    RtlZeroMemory(&ioStatus, sizeof(ioStatus));
    irp = IoBuildDeviceIoControlRequest(
        IOCTL_UART_H4_PROBE_GET_AUDIO_INTERFACE,
        deviceObject,
        NULL,
        0,
        &m_HciAudio,
    ...
    if (!NT_SUCCESS(status) ||
        m_HciAudio.Version != UART_H4_AUDIO_INTERFACE_VERSION ||
        m_HciAudio.Size < sizeof(m_HciAudio) ||
        m_HciAudio.Context == NULL ||
        m_HciAudio.Reference == NULL ||
        m_HciAudio.Dereference == NULL ||
        m_HciAudio.StreamOpen == NULL ||
        m_HciAudio.StreamOpenComplete == NULL ||
        m_HciAudio.StreamClose == NULL ||
        m_HciAudio.ReadCapture == NULL ||
        m_HciAudio.QueueRender == NULL)
    {
        if (NT_SUCCESS(status) &&
            m_HciAudio.Context != NULL &&
            m_HciAudio.Dereference != NULL)
        {
            m_HciAudio.Dereference(m_HciAudio.Context);
        }
        RtlZeroMemory(&m_HciAudio, sizeof(m_HciAudio));
        ObDereferenceObject(fileObject);
        return;
    ...
    m_HciAudioFileObject = fileObject;
    m_HciAudioInterfaceAcquired = TRUE;
}

//=============================================================================
#pragma code_seg("PAGE")
VOID CMiniportWaveRTStream::ReleaseHciAudioInterface()
{
    PAGED_CODE();
    if (m_HciAudioInterfaceAcquired)
    {
        m_HciAudio.Dereference(m_HciAudio.Context);
        m_HciAudioInterfaceAcquired = FALSE;
    }
    RtlZeroMemory(&m_HciAudio, sizeof(m_HciAudio));
    if (m_HciAudioFileObject != NULL)
    {
        ObDereferenceObject(m_HciAudioFileObject);
        m_HciAudioFileObject = NULL;
    }
}

//=============================================================================
#pragma code_seg()
NTSTATUS CMiniportWaveRTStream::OpenHciAudioStream()
{
    NTSTATUS status;

    if (!m_HciAudioInterfaceAcquired || m_HciAudioStreamOpen)
    {
        return STATUS_SUCCESS;
    }

    status = m_HciAudio.StreamOpen(m_HciAudio.Context);
    if (NT_SUCCESS(status))
    {
        m_HciAudioStreamOpen = TRUE;
    }
    return status;
}
...
//=============================================================================
#pragma code_seg()
VOID CMiniportWaveRTStream::CompleteHciAudioStreamOpen()
{
    if (m_HciAudioInterfaceAcquired && m_HciAudioStreamOpen)
    {
        m_HciAudio.StreamOpenComplete(m_HciAudio.Context);
    }
}

//=============================================================================
#pragma code_seg()
VOID CMiniportWaveRTStream::CloseHciAudioStream()
{
    if (m_HciAudioInterfaceAcquired && m_HciAudioStreamOpen)
    {
        m_HciAudio.StreamClose(m_HciAudio.Context);
        m_HciAudioStreamOpen = FALSE;
    }
}
#endif
        ...
        case KSSTATE_RUN:
            ...
            if (m_pMiniport->IsSidebandDevice())
            {
                if (m_SidebandStarted == FALSE)
                {
                    ...
                    //
#ifdef BLUETOOTH_MIC_MAC_HCI_AUDIO_BRIDGE
                    ntStatus = OpenHciAudioStream();
                    IF_FAILED_ACTION_JUMP(
                        ntStatus,
                        DPF(D_ERROR, ("SetState: KSSTATE_RUN, HCI audio StreamOpen failed, 0x%x", ntStatus)),
                        Done);
#endif
                    ntStatus = sidebandDevice->StreamStart(m_pMiniport->m_DeviceType);
#ifdef BLUETOOTH_MIC_MAC_HCI_AUDIO_BRIDGE
                    CompleteHciAudioStreamOpen();
#endif
                    if (!NT_SUCCESS(ntStatus))
                    {
...
--*/
{
    ...
    if (m_HciAudioInterfaceAcquired)
    {
        while (ByteDisplacement > 0)
        {
            ...
            ULONG copied = 0;
            if (m_HciAudioStreamOpen)
            {
                copied = m_HciAudio.ReadCapture(
                    m_HciAudio.Context,
            ...
            {
                RtlZeroMemory(
                    m_pDmaBuffer + bufferOffset + copied,
                    runWrite - copied);
            }
