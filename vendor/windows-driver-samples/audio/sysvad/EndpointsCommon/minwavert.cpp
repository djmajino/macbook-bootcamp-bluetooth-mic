...
#include "simple.h"
#include "minwavert.h"
#ifdef BLUETOOTH_MIC_MAC_WB_MODE_FIX
...
#ifdef BLUETOOTH_MIC_MAC_WB_MODE_FIX
        // The upstream sample always selects the 8 kHz mode-default table
        // here, even after the runtime format table switches to wideband.
        // Keep the processing-mode default format aligned with the current
        // dynamic pin format so the Windows audio engine can open the pin.
        const bool wideband =
            PinId < m_DeviceFormatsAndModesCount &&
            m_DeviceFormatsAndModes[PinId].WaveFormats != NULL &&
            m_DeviceFormatsAndModes[PinId].WaveFormatsCount > 0 &&
            m_DeviceFormatsAndModes[PinId].WaveFormats[0].WaveFormatExt.Format.nSamplesPerSec >= 16000;
        if (m_pSidebandDevice->IsNRECSupported())
        {
            modes = wideband ? BthHfpMicWBPinSupportedDeviceModesNrec : BthHfpMicPinSupportedDeviceModesNrec;
            numModes = wideband ? ARRAYSIZE(BthHfpMicWBPinSupportedDeviceModesNrec) : ARRAYSIZE(BthHfpMicPinSupportedDeviceModesNrec);
        }
        else
        {
            modes = wideband ? BthHfpMicWBPinSupportedDeviceModesNoNrec : BthHfpMicPinSupportedDeviceModesNoNrec;
            numModes = wideband ? ARRAYSIZE(BthHfpMicWBPinSupportedDeviceModesNoNrec) : ARRAYSIZE(BthHfpMicPinSupportedDeviceModesNoNrec);
        }
#else
        if (m_pSidebandDevice->IsNRECSupported())
        {
